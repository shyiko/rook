...
public class TCPReverseProxy {
        ...
        for (; selector.isOpen(); selector.select()) {
            ...
            for (SelectionKey key : keys) {
                ...
                if (key.isAcceptable()) {
                    ...
                    if (logger.isTraceEnabled()) {
                        logger.trace("Established new connection " + System.identityHashCode(remoteSocketChannel) +
                    ...
                    try {
                        ...
                        if (x == -1) {
                            if (logger.isTraceEnabled()) {
                                logger.trace("Closed connection " + System.identityHashCode(targetChannel));
                            }
    ...
    public void unbind() throws IOException {
        ...
        if (logger.isTraceEnabled()) {
            logger.trace("Released port " + port);
        }
        resetInternalState();
    }

    private void closeQuietly(Closeable... arrayOfCloseable) {
        for (Closeable _closeable : arrayOfCloseable) {
            try {
                _closeable.close();
            } catch (IOException e) {
                e.printStackTrace();
            }
            closeable.remove(_closeable);
        }
    }
